...
impl_unary_math!(math_abs_fn, "abs", |x: f64| x.abs());
...
impl_unary_math!(math_ceil_fn, "ceil", |x: f64| x.ceil());
impl_unary_math!(math_floor_fn, "floor", |x: f64| x.floor());
impl_unary_math!(math_trunc_fn, "trunc", |x: f64| x.trunc());
...
crate::std_fn! {
    "std:math", "round", Type::FUNCTION_NUMBER_TO_NUMBER,
...
impl_binary_math!(math_min_fn, "min", |x: f64, y: f64| x.min(y));
impl_binary_math!(math_max_fn, "max", |x: f64, y: f64| x.max(y));
