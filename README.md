...
JSON support is available via the `std:json` module. `json::stringify`
serialises scalars, strings, arrays, maps (string keys only), tuples, and
structs. `json::parse` decodes scalars, strings, arrays, and objects —
tuples / structs come back as arrays / maps, so a roundtrip through JSON
loses the nominal shape.
...
struct Point { x: number, y: number }
var p = Point { x = 1, y = 2 };
print(json::stringify(p));            # {"x":1,"y":2}
